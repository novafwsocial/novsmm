...
NOVSMM uses Prisma ORM with SQLite for development and PostgreSQL for production. The schema includes 32 models with 40+ indexes optimized for the platform's query patterns.

## Schema Files

| File | Purpose |
|------|---------|
| `prisma/schema.prisma` | Active schema (SQLite dev with Json columns) |
| `prisma/schema.postgres.prisma` | PostgreSQL production schema (enums, Decimal, JsonB, VarChar) |
