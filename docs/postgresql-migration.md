...
# Replace with the PostgreSQL schema
cp prisma/schema.postgres.prisma prisma/schema.prisma

# Verify the provider is postgresql
...
# Create the initial migration (creates all tables in PostgreSQL)
bun run prisma migrate dev --name init_postgresql
...
If something goes wrong and you need to rollback to SQLite:

```bash
# Restore the SQLite schema
