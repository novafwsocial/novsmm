...
if ! docker compose -f docker-compose.lowmem.yml exec -T web bunx prisma migrate deploy 2>/dev/null; then
  if [ "${ALLOW_DB_PUSH:-false}" = "true" ]; then
    warn "migrate deploy failed; ALLOW_DB_PUSH=true, applying explicit db:push"
    docker compose -f docker-compose.lowmem.yml exec -T web bun run db:push
  else
    fail "migrate deploy failed; refusing implicit db:push (set ALLOW_DB_PUSH=true only for a disposable SQLite database)"
    exit 1
  fi
fi
ok "Database ready"
