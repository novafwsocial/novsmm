...
if [ -f "prisma/schema.prisma" ]; then
  ...
  if echo "$PROVIDER" | grep -q "postgresql"; then
    ...
    warn "Schema activo usa SQLite — cambia explícitamente el provider del esquema canónico antes del deploy"
  else
...
if [ -f "prisma/schema.postgres.prisma" ]; then
  if [ -d "prisma/migrations" ] && find prisma/migrations -mindepth 2 -maxdepth 2 -type f -name migration.sql -print -quit | grep -q .; then
    ok "Directorio de migraciones Prisma presente"
  else
    fail "No hay migraciones Prisma versionadas; crea y revisa una migración en staging antes del deploy"
  fi
  warn "schema.postgres.prisma existe como referencia heredada; no se usa automáticamente"
else
  warn "schema.postgres.prisma no está presente (no es requisito del esquema canónico)"
fi
