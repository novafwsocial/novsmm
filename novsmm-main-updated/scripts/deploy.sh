...
# Schema PostgreSQL
# prisma/schema.prisma is the canonical schema used by the build and by
# migrations. Never silently replace it with schema.postgres.prisma: that
# legacy reference may not contain the current application models/fields.
PROVIDER=$(awk '/^datasource db[[:space:]]*\{/{in_ds=1;next} in_ds && /provider[[:space:]]*=/{print; exit} in_ds && /^\}/{in_ds=0}' prisma/schema.prisma)
if echo "$PROVIDER" | grep -q '"postgresql"'; then
  ok "Schema canónico usa PostgreSQL"
else
  fail "prisma/schema.prisma no usa PostgreSQL; corrige el provider antes del deploy"
  exit 1
fi
...
if docker compose exec -T web bunx prisma migrate deploy 2>&1; then
  ok "Migraciones aplicadas"
else
  warn "migrate deploy failed; no automatic schema push will be attempted"
  fail "migrate deploy falló; se detiene el deploy y no se ejecuta db:push automáticamente"
  exit 1
fi
...
if [ "$SEED" = true ]; then
  ...
  log "Ejecutando seed..."
  SEED_OUTPUT=$(docker compose exec -T web bun run seed 2>&1)
  echo "$SEED_OUTPUT" | grep -E "✓|Admin|password|⚠️" | head -10
  
  # Extraer password del admin
  if echo "$SEED_OUTPUT" | grep -q "Generated admin password"; then
    warn "Admin seed completed; password is intentionally not printed in deploy logs"
    warn "Retrieve it through the configured secure secret channel and rotate it after first login"
  fi
  ok "Seed completado"
  DEPLOY_PHASE="seeded"
fi
...
if [ "$MIGRATE_SQLITE" = true ]; then
  ...
  if [ -f "db/custom.db" ]; then
    ...
    SQLITE_DATABASE_URL="file:./db/custom.db" docker compose exec -T -e SQLITE_DATABASE_URL="file:./db/custom.db" web bun prisma/migrate-sqlite-to-postgres.ts 2>&1 | tail -20
