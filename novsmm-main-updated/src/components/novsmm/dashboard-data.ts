...
export const REVENUE_SERIES = Array.from({ length: 30 }, (_, i) => ({
  d: i,
  revenue: 1800 + Math.sin(i / 2.2) * 600 + i * 45 + seededNoise(i, 0) * 200,
  orders: 80 + Math.sin(i / 2.6) * 22 + i * 1.5 + seededNoise(i, 1) * 8,
}));

export const HOURLY_ORDERS = Array.from({ length: 24 }, (_, i) => ({
  h: `${i}:00`,
  v: 30 + Math.sin(i / 2.4) * 28 + seededNoise(i, 2) * 18 + (i > 8 && i < 22 ? 30 : 0),
}));
