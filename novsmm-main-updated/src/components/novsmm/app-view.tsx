...
function setPostPaymentGrace() {
  if (typeof window === "undefined") return;
  sessionStorage.setItem("novsmm_post_payment", Date.now().toString());
}

function isPostPaymentGraceActive() {
  if (typeof window === "undefined") return false;
  const ts = sessionStorage.getItem("novsmm_post_payment");
  return !!ts && Date.now() - parseInt(ts, 10) < 10_000;
}
...
function useUrlParamHandlers() {
  ...
  useEffect(() => {
    ...
    if (topup === "success" || topup === "cancelled" || topup === "pending") {
      setPostPaymentGrace();
      stripParam("topup");
    }
  }, []);
...
export function AppView() {
  // Render the landing tree from this client boundary. Passing it through a
  // server-page prop leaves the HTML visible but can skip event hydration on
  // mobile browsers (menu/CTA controls become inert).
  const landing: ReactNode = <LandingContent />;
  ...
  useEffect(() => {
    // Preload the two chunks the user sees first after auth so the dashboard
    // doesn't sit on the skeleton while the browser fetches them lazily.
    void import("./dashboard-shell");
    void import("./dashboard-home");
  }, []);
  ...
  useEffect(() => {
    ...
    const inPostPaymentGrace = isPostPaymentGraceActive();
    ...
    if (inPostPaymentGrace && isAuthed) {
      // Session detected during payment return grace period — clear the flag
      sessionStorage.removeItem("novsmm_post_payment");
    }
    ...
    if (!isAuthed && !isLoading && (view === "dashboard" || view === "onboarding")) {
      if (inPostLoginGrace) {
        ...
        refetchSession();
      } else if (inPostPaymentGrace) {
        // Payment return grace period active but the session is still
        // rehydrating — keep trying before falling back to landing.
        refetchSession();
      } else {
  ...
  if ((isLoading && !isAuthed) || (!isAuthed && inPostPaymentGrace)) {
    // While the session is rehydrating after a payment redirect, keep the
    // user out of the public landing page so the return feels continuous.
    motionKey = "loading";
    content = (
      <div className="flex min-h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-3 text-center">
          <Loader2 className="h-6 w-6 animate-spin text-primary" />
          <p className="text-sm text-muted-foreground">Returning to your wallet…</p>
        </div>
      </div>
    );
