  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <RevealStagger stagger={0.05} className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <RevealItem><AdminStat icon={<Users className="h-4 w-4" />} label="Total users" value={(s?.totalUsers ?? 0).toLocaleString()} delta="live" /></RevealItem>
        <RevealItem><AdminStat icon={<Activity className="h-4 w-4" />} label="Orders (24h)" value={(s?.orders24h ?? 0).toLocaleString()} delta="live" /></RevealItem>
        <RevealItem><AdminStat icon={<CreditCard className="h-4 w-4" />} label="Revenue (30d)" value={`$${(s?.revenue30d ?? 0).toFixed(2)}`} delta="live" /></RevealItem>
        <RevealItem><AdminStat icon={<Server className="h-4 w-4" />} label="Active services" value={(s?.services ?? 0).toLocaleString()} delta={`${s?.providers ?? 0} providers`} /></RevealItem>
      </RevealStagger>
      </DashReveal>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Reveal blur className="lg:col-span-2">
          <div className="rounded-2xl border border-border/60 bg-background p-5 sm:p-6">
            ...
            <div className="mt-1 text-2xl font-semibold tabular-nums">${(s?.revenue30d ?? 0).toFixed(2)}</div>
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const payload = hasNewCreds
        ? { method: method.name, methodId: method.id, credentials }
        : { method: method.name, methodId: method.id };
...
const AdminOrders = memo(function AdminOrders() {
  ...
  const resyncOrder = useResyncAdminOrder();
  const [showCreate, setShowCreate] = useState(false);
          ...
          <div className="overflow-x-auto nov-scroll">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border/60">
                  ...
                  <tr key={o.id} className="table-row-hover transition-colors hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3">
                      {o.status === "completed" || o.status === "cancelled" ? (
                        ...
                        <button
                          onClick={() => resyncOrder.mutate(o.id)}
                          disabled={resyncOrder.isPending || !o.providerName}
                          className="inline-flex items-center gap-1 rounded-full border border-border bg-background px-3 py-1.5 text-[11px] font-medium text-foreground transition-colors hover:bg-muted disabled:cursor-not-allowed disabled:opacity-60"
                          title={o.providerName ? "Force provider status sync" : "No provider reference available"}
                        >
                ...
                {orders.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center text-sm text-muted-foreground">No orders yet</td>
                  </tr>
                )}
              </tbody>
            </table>
