...
export function DashboardHome() {
  ...
  const previewOrders = recentOrders.slice(0, 4);
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6">
      ...
      <DashReveal>
      <RevealStagger stagger={0.06} className="grid grid-cols-2 gap-2 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
        <RevealItem>
          <StatCard
      ...
      <DashReveal delay={0.05}>
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          <Reveal blur className="lg:col-span-2">
            <div className="rounded-2xl border border-border/60 bg-background p-4 sm:p-6">
              ...
              <div className="chart-container mt-4 h-[180px] w-full sm:mt-5 sm:h-[220px]">
        ...
        <div className="flex flex-col gap-3">
          <Reveal blur delay={0.06}>
            <div className="rounded-2xl border border-border/60 bg-gradient-to-br from-foreground to-foreground/90 p-4 text-background sm:p-5">
        ...
        <div className="rounded-2xl border border-border/60 bg-background p-4 sm:p-6">
          ...
          <div className="mt-4 flex flex-col gap-1">
            {previewOrders.length === 0 ? (
              ...
              previewOrders.map((o: any, i: number) => (
                <div
                  key={o.id}
                  className="fm-fade-up flex flex-col gap-2 rounded-xl px-2 py-2 transition-colors hover:bg-muted/40 sm:flex-row sm:items-center"
          ...
          {recentOrders.length > previewOrders.length && (
            <div className="mt-3 text-center text-[11px] text-muted-foreground">
              Showing latest {previewOrders.length} of {recentOrders.length} orders.
            </div>
          )}
        </div>
      </Reveal>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Reveal blur>
            ...
            <div className="mt-3 flex flex-col gap-2">
              {previewFavorites.length > 0 ? previewFavorites.map((f: any) => (
              ...
              {favorites.length > previewFavorites.length && (
                <div className="text-center text-[11px] text-muted-foreground">
                  Showing latest {previewFavorites.length} favorite services.
                </div>
              )}
            </div>
          </div>
        ...
        <Reveal blur delay={0.06}>
          <div className="rounded-2xl border border-border/60 bg-background p-4 sm:p-5">
            ...
            <div className="mt-3 flex flex-col gap-2">
              {previewTickets.length > 0 ? previewTickets.map((t: any) => (
...
function StatCard({
  ...
  return (
    <div className="stat-card-3d group relative h-full overflow-hidden rounded-2xl border border-border/60 bg-background p-3 transition-shadow hover:nov-ring-lg sm:p-5">
      <div className="flex items-start justify-between">
        <span className={cn("flex h-8 w-8 items-center justify-center rounded-lg sm:h-9 sm:w-9", TINTS[tint])}>
          {icon}
        </span>
        <span
          className={cn(
            "inline-flex items-center gap-0.5 text-[10px] font-medium sm:text-[11px]",
      ...
      <div className="mt-2 text-xl font-semibold tabular-nums sm:mt-3 sm:text-3xl">{value}</div>
      <div className="mt-0.5 text-[11px] text-muted-foreground sm:text-xs">{label}</div>
    </div>
  );
}
