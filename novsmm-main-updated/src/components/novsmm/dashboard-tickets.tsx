            ...
            >
              {tickets.length === 0 ? (
                ...
                <div className="overflow-hidden rounded-2xl border border-border/60 bg-background">
                  ...
                  <div className="max-h-[58vh] overflow-y-auto nov-scroll">
                    {filtered.length === 0 ? (
...
function TicketRow({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex w-full flex-col gap-1 border-b border-border/60 p-3 text-left transition-colors sm:p-3.5",
...
const ConversationBody = forwardRef<HTMLDivElement, { active: any }>(
  function ConversationBody({ active }, ref) {
    return (
      <div
        ref={ref}
        className="flex-1 space-y-3 overflow-y-auto bg-muted/20 p-3 nov-scroll sm:p-4"
      >
        {active.messages?.map((m: any, i: number) => (
          ...
          >
            <div
              className={cn(
                "max-w-[82%] rounded-2xl px-3.5 py-2.5 text-sm sm:max-w-[75%]",
                m.sender === "user"
...
function ConversationComposer({
  ...
  return (
    <div className="border-t border-border/60 p-3">
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-xl border border-border bg-background p-2 transition-shadow focus-within:shadow-[0_0_0_4px_rgba(0,82,255,0.12)]">
        ...
        <textarea
          ...
          className="min-h-12 max-h-32 flex-1 resize-none bg-transparent px-2 py-2 text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none"
        />
        {canUseCannedReplies && (
          <CannedRepliesButton
            onInsert={(body, id) => {
        ...
        <motion.button
          ...
          className={cn(
            "flex h-9 w-9 items-center justify-center rounded-lg transition-all sm:h-8 sm:w-8",
            input.trim() && !pending
