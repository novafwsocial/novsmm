...
export function Footer() {
  ...
  const handleLink = (link: FooterLink) => {
    ...
    if (link.anchor) {
      ...
      if (authed) {
        ...
        setTimeout(() => {
          scrollToAnchorWhenReady(link.anchor!);
        }, 80);
      } else {
        scrollToAnchorWhenReady(link.anchor);
      }
  ...
  return (
    <footer className="relative mt-auto overflow-hidden border-t border-border bg-background pb-24 sm:pb-0">
      {/* Final CTA band */}
      ...
      <div className="border-t border-border/60">
        <div className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-16">
          <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 sm:gap-x-8 lg:grid-cols-6 lg:gap-x-10">
          ...
          <div className="mt-8 flex flex-col items-start justify-between gap-4 border-t border-border/60 pt-6 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-3">
              {/* MOB-1d-001 FIX: replaced the broken button (that navigated to
                  register on mobile) with the actual LanguageSwitcher component.
                  This shows a dropdown with all 4 supported languages (EN, ES,
                  PT, FR) and works for both authed and non-authed users. */}
