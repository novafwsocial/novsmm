...
import { useOrders, useOrder, useRepeatOrder, useCancelOrder, useCreateTicket, useRefillOrder } from "@/hooks/use-api";
...
export function DashboardOrders() {
  ...
  const [selectedOrderPublicId, setSelectedOrderPublicId] = useState<string | null>(null);
  ...
  const orderDetail = useOrder(selectedOrderPublicId);

  useEffect(() => {
    if (typeof window === "undefined") return;
    const orderParam = new URLSearchParams(window.location.search).get("order");
    if (orderParam) {
      setSelectedOrderPublicId(orderParam);
    }
  }, []);

  const openOrder = (publicId: string) => {
    setSelectedOrderPublicId(publicId);
    if (typeof window === "undefined") return;
  ...
  const selectedOrder = orderDetail.data?.order ?? filtered.find((o: any) => o.publicId === selectedOrderPublicId) ?? null;
  const selectedOrderLoading = !!selectedOrderPublicId && orderDetail.isLoading && !selectedOrder;
    ...
    <div className="flex flex-col gap-6">
      ...
      <AnimatePresence mode="wait">
        ...
        {selectedOrderLoading && !selectedOrder && (
          <OrderDetailLoadingDrawer onClose={closeOrder} />
        )}
      </AnimatePresence>
    </div>
  );
...
function OrderDetailLoadingDrawer({ onClose }: { onClose: () => void }) {
  return (
    <>
      <motion.div
        initial={{ opacity: 0 }}
      ...
      <motion.aside
        ...
        exit={{ x: "100%" }}
        transition={{ type: "spring", stiffness: 320, damping: 36 }}
        className="fixed right-0 top-0 z-[81] flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-border/60 bg-background nov-scroll"
      >
        <div className="sticky top-0 z-10 flex items-center justify-between gap-2 border-b border-border/60 bg-background/95 px-5 py-4 backdrop-blur-sm">
          <div>
            <div className="font-mono text-sm font-semibold text-foreground">Loading order…</div>
            <div className="text-[11px] text-muted-foreground">Fetching full details</div>
          </div>
          <button
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-full bg-background/80 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
