    ...
    <div className="relative flex min-h-[100dvh] flex-col bg-background">
      ...
      <aside
        className={cn(
          ...
          isImpersonating ? "top-12 h-[calc(100dvh-3rem)]" : "top-0 h-[100dvh]"
        )}
      >
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 z-50 flex h-[100dvh] w-[min(85vw,320px)] flex-col border-r border-border bg-background lg:hidden"
            >
              <div className="flex h-16 items-center justify-between px-5">
                <a
                  onClick={() => {
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header
          className={cn(
            "sticky z-30 flex h-16 items-center gap-2 border-b border-border/60 bg-background/80 px-3 backdrop-blur-xl sm:gap-3 sm:px-6",
        ...
        >
          ...
          <button
            type="button"
            onClick={openPalette}
            className="flex flex-1 items-center gap-2 rounded-full border border-border bg-muted/40 px-3 py-2 text-sm text-muted-foreground transition-colors hover:border-primary/40 hover:bg-background"
            aria-label="Open command palette"
          >
            <Search className="h-3.5 w-3.5" />
            <span className="w-full text-left text-foreground/80">
              <span className="sm:hidden">Search…</span>
              <span className="hidden sm:inline">Search orders, services, clients…</span>
            </span>
          ...
          {(() => {
            ...
            const initials = (user?.name || user?.email || "U")
              ...
              .toUpperCase();
            return (
              <div className="hidden h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-primary to-primary/60 text-xs font-semibold text-primary-foreground sm:flex" aria-label={`Account: ${user?.name ?? user?.email ?? "User"}`}>
                {initials}
              </div>
            );
          })()}
        ...
        {/* Content */}
        <main className="flex-1 overflow-x-hidden p-4 sm:p-6 lg:p-8 max-[767px]:pb-32 max-[767px]:pr-24">
