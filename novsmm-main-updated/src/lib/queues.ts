 ...
 *   - provider.status.sync    → Poll provider-backed orders until terminal
...
> = {
  ...
  "provider.status.sync": { concurrency: 3, maxRetries: 5, backoffMs: 15000 },
...
const JOB_HANDLERS: Record<QueueName, (data: any) => Promise<void>> = {
  ...
  "provider.status.sync": async (data) => {
    const { syncProviderOrderStatus } = await import("./provider-status-sync");
    await syncProviderOrderStatus(data.orderId);
  },
  "loyalty.reconcile": async (data) => {
    const { reconcileAchievements } = await import("@/lib/services/loyalty.service");
