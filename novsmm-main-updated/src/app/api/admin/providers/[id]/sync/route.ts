...
export async function POST(
  ...
  // The provider name is user-controlled metadata and must never widen the
  // network allowlist. Only the parsed hostname is authoritative; otherwise
  // an admin/provider row named "huntsmm" could exfiltrate HUNTSMM_API_KEY to
  // an arbitrary public endpoint.
  const isHuntSmm = ALLOWED_HOSTS.includes(hostname);
