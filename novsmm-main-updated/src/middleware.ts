...
function addSecurityHeaders(res: NextResponse, nonce?: string, isHttps = false) {
  ...
  // Development clients (including the in-app browser) can strip nonce
  // attributes from streamed RSC markup before execution. Keep the strict
  // nonce-only policy in production, but allow inline dev bootstrap scripts
  // so hydration and interactive mobile controls remain testable locally.
  const scriptSrc = isProduction
    ? nonce
      ? `'self' 'nonce-${nonce}' https://www.paypal.com https://www.paypalobjects.com`
  ...
  // `upgrade-insecure-requests` is correct for production HTTPS, but it
  // breaks local HTTP development by upgrading localhost JS chunks to a
  // TLS endpoint that does not exist. Keep the directive production-only.
