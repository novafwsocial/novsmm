...
export function useOrder(orderId?: string | null) {
  return useQuery({
    ...
    queryFn: () => api.get<{ order: any }>(`/api/orders/${encodeURIComponent(orderId ?? "")}`),
    enabled: !!orderId,
    staleTime: 30 * 1000,
  });
}
...
export function useResyncAdminOrder() {
  const qc = useQueryClient();
  const { toast } = useToast();
  return useMutation({
    mutationFn: (orderId: string) =>
      api.post<{ message: string; result: any }>(`/api/admin/orders/${orderId}/resync`, {}),
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ["admin-overview"] });
      qc.invalidateQueries({ queryKey: ["orders"] });
      toast({ title: "Order resync requested", description: data.message });
    },
    onError: (e: Error) => toast({ title: "Resync failed", description: e.message, variant: "destructive" }),
  });
}
