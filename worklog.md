...
---
Task ID: SPRINT-3-CHILD-PANELS-FAILOVER
Agent: main (Z.ai Code)
Task: Sprint 3 — Child Panel self-service + Provider Failover. Implementar
(1) compra self-service de white-label sub-panels con auto-provisioning de
subdomain + API key, y (2) multi-provider fulfillment con failover
automático (priority-ordered provider chain).

Work Log:
- Leí el worklog anterior (Sprint 2) y el schema actual. Estudié los patrones
  existentes: subscriptions/route.ts (atomic balance debit),
  admin/licenses/route.ts (API key encrypt-once + bcrypt hash + SHA-256
  lookupHash), crypto-utils.ts (AES-256-GCM), ids.ts (nextPublicId), notify.ts,
  queues.ts, orders.ts (existing single-provider HuntSMM call), huntsmm.ts,
  dashboard-subscriptions.tsx (dashboard tab pattern), admin-panel.tsx
  (ServiceModal, useAdminProviders), app-store.ts / dashboard-shell.tsx /
  app-view.tsx (tab wiring).

Section 1: Prisma schema — `prisma/schema.prisma` (MODIFIED)
- Added `serviceProviders ServiceProvider[]` relation to both Provider and
  Service models.
- Added `smmChildPanels ChildPanel[]` relation to User.
- Added new `ServiceProvider` model — join table for multi-provider with
  priority (1=primary, 2+=fallback), providerServiceId, cost.
  `@@unique([serviceId, providerId])` + indexes on `(serviceId, priority)`
  and `(providerId)`.
...
- Ran `bunx prisma db push --accept-data-loss` → success (SQLite, 41ms).
  Prisma Client regenerated.

Section 2: Child Panels API (NEW)
- `src/app/api/child-panels/route.ts`:
  ...
  - POST: purchase/create. Zod validation: name (1-50), subdomain (3-30 chars,
    ...
    (reseller|agency|enterprise, default reseller), markupPercent (0-100,
    default 20), monthlyDays (1-365, default 30). Plan fees: reseller $49,
    agency $149, enterprise $499. totalCost = monthlyFee × monthlyDays / 30.
    Pre-flight subdomain uniqueness check, conditional atomic balance debit
    (same MVCC pattern as orders/subscriptions), publicId via
    nextPublicId("CP", 500), tx via nextPublicId("TX", 8842). API key
    generated as `nvsk_child_${crypto.randomBytes(20).toString('hex')}`,
    bcrypt-hashed (cost 12) for apiKeyHash, SHA-256 lookupHash, AES-256-GCM
    encrypted for apiKey column. Ledger transaction + notification + audit on
    success. Returns 201 with panel + PLAINTEXT apiKey (shown ONCE, like
    license keys).
- `src/app/api/child-panels/[id]/route.ts`:
  - GET: single panel detail, ownership check via
    `findFirst({ where: { id, userId } })`, single 404 to avoid leaking IDs.
  - PATCH: update name / markupPercent / status. Status transition matrix:
    active→suspended/cancelled, suspended→active/cancelled,
    cancelled→(terminal). Idempotent same-status updates. Audit + notification
    on status change.
  - DELETE: soft-delete (status=cancelled). Row kept for billing history.

Section 3: Provider Failover (NEW + MODIFIED)
- `src/lib/provider-failover.ts` (NEW):
  - `fulfillWithFailover(order)` — tries the service's providers in priority
    ...
    using mapping.providerServiceId (or fall back to extracting `[N]` from
    the service name). On success: stamp the order with `providerName #orderId`,
  ...
  - Backward compat: if no ServiceProvider mappings exist, falls back to the
    legacy single-provider HuntSMM flow (same behavior as pre-Sprint 3).
- `src/lib/orders.ts` (MODIFIED):
  - Removed direct `placeHuntSMMOrder` + `extractProviderServiceId` imports
    (now encapsulated in provider-failover.ts).
  - Replaced the single-provider HuntSMM block with a call to
    `fulfillWithFailover(order)`. Added `serviceId` to the `findUnique` select.
    If failover returns a result → return early. If null → fall through to
    the existing simulated setTimeout steps (unchanged).
...
- `src/app/api/admin/services/route.ts` (MODIFIED):
  - GET: now includes `serviceProviders: { include: { provider: true },
    orderBy: { priority: "asc" } }` so the admin UI has the full mapping
    list per service.
...
- `src/components/novsmm/app-view.tsx` (MODIFIED): added
  `DashboardChildPanels` dynamic import + render line (after subscriptions,
  before wallet).
- `src/components/novsmm/dashboard-child-panels.tsx` (NEW): full dashboard tab
  ...
  - CreatedKeyBanner: shown once after creation, with the plaintext API key
    in a `<code>` block + a Copy button (clipboard API). Emphasised with
    emerald border + KeyRound icon. Dismissible.
  - ChildPanelCard: name, publicId (mono), subdomain (as link with
    ...
    (emerald), monthlyFee, paidUntil (red if expired), status badge (4
    states: active/suspended/expired/cancelled). Actions: Edit (name+markup),
    Suspend (if active), Resume (if suspended), Cancel (if active/suspended,
    with confirm).
  ...
  - CreateChildPanelModal: name input, subdomain input (with `.novsmm.com`
    ...
    Enterprise $499/mo — each with tagline), markupPercent slider (0-100%),
    duration buttons (30/90/365 days) + custom number input. Live cost
    estimate card: monthly fee × days / 30 = total, balance check,
    insufficient-balance warning. Submit disabled until valid + sufficient.
  - EditChildPanelModal: name + markupPercent slider.

Section 6: React Query hooks — `src/hooks/use-api.ts` (MODIFIED)
- Added 4 hooks near `useUpdateSmmSubscription`:
  - `useChildPanels()` — useQuery, 60s refetch, GET /api/child-panels.
...
---
Task ID: ADMIN-FIX-BATCH-2
Agent: full-stack-developer (Z.ai Code)
Task: Apply 6 remaining fixes (4 medium + 2 low) for full admin panel sync

Work Log:

### Pre-flight
- Read `/home/z/my-project/worklog.md` + `agent-ctx/ADMIN-FIX-BATCH-1-full-stack-developer.md`
  for context. Noted Batch-1 explicitly deferred the 6 fixes addressed here.
- Read `prisma/schema.prisma` to confirm actual model fields:
  - `Provider`: only `id, name, apiUrl, apiKey, status, latency, services, serviceProviders,
    createdAt, updatedAt`. **No `lastSyncAt`** column — `updatedAt` (auto-bumped by Prisma's
    `@updatedAt`) serves the same role. Did NOT add a column (would require a migration; the
    spec said "if the schema doesn't have `category`/`type`/`description` fields, just render
    all keys with text inputs" — same pragmatic approach for `lastSyncAt`).
  - `Setting`: only `id, key, value`. No category/type/description — inferred both from the
    key prefix and value shape.
  - `License.plan`: comment lists `reseller | agency | enterprise | white_label` (default
    "reseller"). The admin UI issue-modal uses the same 4 values. The task spec's
    `["starter", "pro", "business", "enterprise"]` don't appear anywhere in the codebase —
    used the actual values per the brief ("Use the actual values you find").
- Read `prisma/sync-huntsmm.ts` (reference) and `src/lib/huntsmm.ts` (no existing
  `getHuntSmmServices` function — only `placeHuntSMMOrder`, `checkHuntSMMOrderStatus`,
  `checkHuntSMMBalance`, `extractProviderServiceId`).

### MEDIUM FIX 1 — Remove German from seed (Option A)
- `prisma/seed-settings.ts`: removed the `{ code: "de", ... }` entry from the languages
  array (down to 4 languages: en, es, pt, fr). Added
  `await db.language.deleteMany({ where: { code: "de" } });` after the upsert loop so
  re-seeding converges. Decision rationale (documented in code): `src/lib/i18n.ts` has
  zero German keys — well below the 80% threshold. Shipping a language row with no UI
  strings would force every label to fall back to English, which is worse than not
  offering the option.
- `src/lib/i18n.ts`: updated the file header comment from `Languages: en, es, pt, fr, de`
  to `Languages: en, es, pt, fr` with a note that German was removed pending a complete
  `de` translation pack.

### MEDIUM FIX 2 — Real provider sync endpoint
- `src/lib/huntsmm.ts`: added `getHuntSmmServices(apiKey, apiUrl?)` that POSTs to the
  HuntSMM `services` action and returns the parsed array. Throws on HTTP errors,
  auth failures (`{ error: "..." }` payloads), and non-array payloads — so the caller
  can mark the provider `degraded` and surface the message.
- `src/app/api/admin/providers/[id]/sync/route.ts`: full rewrite.
  - Reads the Provider row by `[id]`. 404 if not found.
  - If `apiUrl` (or `name`) contains `huntsmm.com` → calls `getHuntSmmServices()` with
    the DB `apiKey` (falls back to `process.env.HUNTSMM_API_KEY`).
  - Upserts each remote service into the Service table by `name` (unique) — the
    `[<remote-id>] <remote-name>` format matches `prisma/sync-huntsmm.ts` so re-syncing
    converges. 30% markup preserved. Detects platform/quality/category using the same
    heuristics as the seed script (copied locally to avoid coupling the route to the
    prisma script's internals).
  ...
  - try/catch wraps the whole flow; on failure sets `provider.status = "degraded"`,
    audits `provider.sync_failed`, returns 502 with the error message. Inner
    `.update().catch(()=>{})` ensures we never re-throw from the catch path.
  - Audits `provider.sync` on success with `{ provider, synced, latency, status }`.
  - Uses the typed `requireAdmin()` result (`user!.id`) — no more `(session!.user as any).id`.

### MEDIUM FIX 3 — Refunds panel filter
- `src/components/novsmm/admin-panel.tsx` `AdminRefunds`: renamed `transactions` →
  `allTransactions` and added a `refundable` derived list filtered by
  `["topup", "sale"].includes(t.type) && t.status === "completed"`. The Refunds table
  now iterates `refundable` instead of all recent transactions.
- Added an amber info panel at the top: "Showing refundable transactions only (top-ups
  and sales). Withdrawals, referral bonuses, fees, and existing refunds are excluded."
- Updated the empty-state row copy and the header count ("refundable transactions")
  to reflect the new semantics.
- Backend: the listing endpoint that feeds this panel is `GET /api/admin/overview`
  (`recentTransactions`, already `status: "completed"`). I deliberately did NOT add a
  `type` filter there — the Overview tab also reads `recentTransactions` and would have
  been affected. Frontend filtering is the surgical fix; no API change needed.

### MEDIUM FIX 4 — Dynamic settings keys
- `src/components/novsmm/admin-panel.tsx` `AdminSettingsTab`: removed the hardcoded
  `editableKeys` array. The component now:
  1. Reads every key from `useAdminSettings()` (already returns
     `{ settings: Record<string, string> }`).
  2. Groups by the prefix before the first "." (platform, fees, limits, security,
     oauth, payments, …) using `useMemo`. Keys without a dot land in a "General" bucket
     sorted last so nothing is hidden.
  3. Renders one card per group, with each setting as either:
     - a `number` Input when the current value parses as a finite number
       (handles `limits.*`, `fees.*`, `security.rateLimitPerMinute`);
     - a `text` Input otherwise;
     - a read-only dashed input for encrypted credential blobs (keys starting with
       `oauth:` or `payments:`) — editing ciphertext from this panel would corrupt
       the creds, and the dedicated Social Auth / Payments tabs are the right place.
  4. Save button sends only the changed values via the existing PATCH (unchanged
     backend).
- The Setting model has no `category`/`type`/`description` fields — per the brief, we
  render all keys with text inputs (plus the numeric + read-only refinements above).

### LOW FIX 5 — License plan enum validation
- `src/lib/validations.ts`: added `LICENSE_PLANS = ["reseller", "agency", "enterprise",
  "white_label"] as const`, `LicensePlan` type, `createLicenseSchema` (Zod, strict), and
  `updateLicenseSchema` (Zod, strict). Both constrain `plan` to the enum.
- `src/app/api/admin/licenses/route.ts`:
  - POST now `safeParse`s the body with `createLicenseSchema`. On failure, if the first
    issue path is `plan`, returns 422 with
    `Invalid plan. Must be one of: reseller, agency, enterprise, white_label` —
    otherwise surfaces the Zod message. Removed the manual `if (!customerName || …)`
    check (now covered by the schema) and the `plan ?? "reseller"` fallback (the
    schema defaults `plan` to `"reseller"`).
  - PATCH now `safeParse`s with `updateLicenseSchema`. Same 422 plan-error handling.
    PATCH now also accepts `plan` as an editable field (was previously silently
    ignored — only `domain/ipAllowlist/maxUsers/maxOrders/expiresAt` + status actions
    were applied).
  - Both handlers switched from `(session!.user as any).id` to the typed `user!.id`.
  - Added safe-JSON-parse to PATCH (was missing — would have thrown 500 on bad body).

### LOW FIX 6 — Social Auth multi-provider UI
- `src/app/api/admin/social-auth/route.ts`: full rewrite.
  - Exports `SOCIAL_AUTH_PROVIDERS = ["google", "facebook", "github", "twitter"] as const`.
  - GET always returns an entry for every supported provider (with `{ configured,
    source: "db" | "env" | null }`) so the UI can render all 4 cards deterministically.
    A provider is `configured` if either (a) a `oauth:<provider>` Setting row exists
    in the DB, or (b) the matching env-var pair (`GOOGLE_CLIENT_ID`/`..._SECRET`,
    `FACEBOOK_*`, `GITHUB_*`, `TWITTER_*`) is set. `source` distinguishes the two so
    the UI can warn that env-var-configured providers can't be disabled from the panel.
  - POST validates `provider` against the enum (was: rejected anything but "google").
    Encrypts `{ clientId, clientSecret }` via `encryptJSON` and upserts the
    `oauth:<provider>` Setting row. Mirrors the creds into `process.env` at runtime
    (preserves the legacy "DB creds take effect immediately" behaviour for Google and
    extends it to the new providers).
  - New DELETE handler — accepts `{ provider }` and `deleteMany`s the
    `oauth:<provider>` Setting row + clears the runtime env override. Powers the UI's
    "Disable" button.
  - All three handlers audit the action.
- `src/components/novsmm/admin-panel.tsx` `AdminSocialAuth`: full rewrite.
  - `SOCIAL_PROVIDERS` constant drives the UI: 4 cards (Google, Facebook, GitHub,
    Twitter/X) each with provider glyph (inline SVG), label, redirect URL
    (`https://novsmm.shop/api/auth/callback/<provider>`), and field placeholders.
  - Per-provider state: `creds[provider] = { clientId, clientSecret }` and
    `statuses[provider] = { configured, source }`.
  - Each card renders: Client ID input, Client Secret input (password), read-only
    Redirect URL with copy-to-clipboard button, "Save & enable"/"Update credentials"
    button (POST), and a "Disable" button (DELETE, only when configured, disabled
    when `source === "env"` because env-var creds can't be removed from the UI).
...
Key changes:
- German language removed from seed + cleanup `deleteMany` (would otherwise leave a
  ghost "Deutsch" option in the language selector with no UI translations).
- Provider sync route now actually fetches the HuntSMM catalog and upserts services
  into the Service table (was: a 50-250ms `setTimeout` that always reported success).
  Failure path marks the provider `degraded` and returns 502.
- Refunds panel now shows only refundable transactions (top-up + sale, completed) —
  previously showed every recent transaction including withdrawals/referrals which
  can't be reversed from this panel.
- Settings tab now renders ALL settings returned by the API (grouped by prefix,
  numeric/encrypted-aware) instead of a hardcoded list of 9 keys.
- License `plan` field is now enum-validated server-side via Zod; POST and PATCH
  both return 422 with a clear "Invalid plan. Must be one of: ..." message.
- Social Auth panel now supports Google + Facebook + GitHub + Twitter, each with
  its own enable/disable flow, Client ID/Secret inputs, and read-only redirect URL
  (with copy button). Backend extended with a DELETE handler to disable a provider.

Files modified:
- `prisma/seed-settings.ts` — German removed from languages array + cleanup deleteMany.
- `src/lib/i18n.ts` — header comment updated (no `de` translation pack).
- `src/lib/huntsmm.ts` — added `getHuntSmmServices()` helper.
- `src/app/api/admin/providers/[id]/sync/route.ts` — real HuntSMM sync, 501 for others,
  degraded-on-failure, audit log.
- `src/components/novsmm/admin-panel.tsx` — AdminRefunds filter + note; AdminSettingsTab
  dynamic grouping; AdminSocialAuth multi-provider UI.
- `src/lib/validations.ts` — added LICENSE_PLANS, LicensePlan, createLicenseSchema,
  updateLicenseSchema.
- `src/app/api/admin/licenses/route.ts` — POST + PATCH now Zod-validate (incl. plan enum),
  PATCH accepts `plan` updates, safe JSON parse, typed user.
- `src/app/api/admin/social-auth/route.ts` — multi-provider GET/POST + new DELETE handler.
...
Issues encountered:
- Task spec for FIX 5 suggested plan enum `["starter", "pro", "business", "enterprise"]`
  but the actual schema/UI values are `["reseller", "agency", "enterprise",
  "white_label"]`. Followed the brief ("Use the actual values you find") — documented
  in code comments.
- Task spec for FIX 2 suggested `audit(req, "provider.sync", { providerId, synced: N })`
  but our `audit()` helper has signature `(userId, action, entity, entityId, metadata)`.
  Used the actual signature: `audit(adminId, "provider.sync", "provider", provider.id,
  { provider, synced, latency, status })`.
- Task spec for FIX 2 mentioned `provider.lastSyncAt = now`. Provider schema has no
  `lastSyncAt` column. Used the existing `updatedAt` (Prisma `@updatedAt`) — auto-bumped
  on the `provider.update()` call.
- Task spec for FIX 4 mentioned `category/type/description` on the Setting model — none
  of those fields exist on the Setting model (only `id, key, value`). Followed the brief's
  fallback ("If the Setting schema doesn't have … fields, just render all keys with text
  inputs") with the small refinements noted above (numeric detection, encrypted
  read-only display).
- Task spec for FIX 6 suggested possibly adding a `provider` field to a `SocialAuth`
  table. No SocialAuth table exists — credentials are already stored as encrypted
  JSON in `Setting` rows keyed `oauth:<provider>`. Extended the existing pattern rather
  than introducing a new table (kept it simple, per the brief).
